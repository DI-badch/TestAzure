...
def parse_page_list(page_list):
    ...
    while p_i < len(page_ranges) and c_i < len(clear_ranges):
        ...
        if p.start < c.start:
            ranges.append(PageRange(p.start, p.end, cleared=False))
            p_i += 1
        else:
            ranges.append(PageRange(c.start, c.end, cleared=True))
            c_i += 1
...
class ContainerProperties(DictMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.immutable_storage_with_versioning_enabled = kwargs.get(
...
class ContainerSasPermissions(object):
    ...
    def __init__(
        self, read=False, write=False, delete=False, list=False, delete_previous_version=False, tag=False, **kwargs
